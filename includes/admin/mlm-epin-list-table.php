...
class EpinReports_List_Table extends WP_List_Table {
    ...
    function get_bulk_actions() {
        ...
        $this->epin_dropdown();
        submit_button(__('Filter', 'mlm'), 'secondary', false, false, array('id' => 'post-query-submit'));
    ...
    function epin_dropdown() {
        ...
        <select name="epin_status">
            ...
            <option value="" <?php echo ($epin_status == '' ? 'selected="selected"' : ''); ?>><?php _e('Choose Option', 'mlm') ?></option>

            <option value="1" <?php echo ($epin_status == '1' ? 'selected="selected"' : ''); ?>><?php _e('Used ePin', 'mlm') ?></option>

            <option value="0" <?php echo ($epin_status == '0' ? 'selected="selected"' : ''); ?>><?php _e('UnUsed ePin', 'mlm') ?></option>
        </select>
        ...
        ?>
        <?php $pro_price_settings = $wpdb->get_results("select * from {$wpdb->prefix}binarymlm_product_price"); ?>
    ...
    function prepare_items() {
        ...
        if ($num > 0) {
         ...
         if(!empty($usersearch)){ 
	          ...
	          {
	          $epin_status.=" AND p_id=$epin_value";
	          }


      		$sql = "SELECT * FROM {$wpdb->prefix}binarymlm_epins where epin_no like '%$usersearch%' $epin_status";
	          ...
	          $i=0;
		          foreach( $rs as $row){
			          $user_id=empty($row['user_key'])?'':$this->letscms_getuseruidbykey($row['user_key']);
	          ...
	          if(isset($epin_status) && ($epin_status!=''))
	          ...
	          if(isset($epin_value) && ($epin_value!=''))
	          {
		          if($status!='')
		          {
			        $status.= " AND p_id=$epin_value";

		          } else {
		          
		          $status=" WHERE p_id=$epin_value";
	          }
          }
 
          $sql = "SELECT * FROM {$wpdb->prefix}binarymlm_epins  $status ORDER BY id ASC";
