...
class Letscms_MLM_Admin_Genealogy {
use letscms_mlmfunction;
  ...
  public function wp_mlm_enqueue_script() {
	...
	function addLeftLeg($key, $add_page_id)
	{
		...
		$str = "[{v:'".$key."ADD',f:'Empty<br><span class=\"leg\">Left leg</span>'},'".$username."',''],";
		return $str;
	}
	...
	{
		$username = $this->letscms_getUsername($key);
		$str = "[{v:'".$key."ADD2',f:'Empty<br><span class=\"leg\">Right leg</span>'},'".$username."',''],";
		return $str;
	}
...
//buildLevelByLevelNetwork() function build the 1st and more level network
	function buildLevelByLevelNetwork($key, $add_page_id, $view_page_id, $counter, $level)
	{
		...
		for($i=0; $i<$counter; $i++)
		{
			...
			{
				$sql = "SELECT username, payment_status, user_key, leg FROM {$wpdb->prefix}binarymlm_users
        	      WHERE parent_key = '".$level[$i]."' AND banned='0' ORDER BY leg DESC";
	...
	//checkPaymentStatus() function check the node user is paid or not
		...
		{
			$paymentStr='<br><a href="?page=binarymlm-user-account&ac=network&k='.$key.'">View</a>';
			if($payment==1)
			{
			return $paymentStr.='<br><span class=\"paid\">PAID</span>';
			}
		}
		...
		}elseif(!empty($_GET['k']) && $_GET['k']!=''){
			$key = $_GET['k'];
		}
		else{
			$key = $this->letscms_get_user_key_admin($username2);
		}
...
public function letscms_admin_genealogy_network($id){
...
<script type="text/javascript" language="javascript">
    ...
    <div style="margin-left:20px;">
		...
		<tr>
		<td align="center">
					<form action="?page=binarymlm-user-account&ac=network" method="post">
